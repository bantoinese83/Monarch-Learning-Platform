...
class GeminiFileSearchService:
    ...
    def upload_and_index_file(
        ...
        try:
            ...
            if not operation.done:
                raise Exception(f"Indexing operation timed out after {max_attempts * poll_interval}s")
    ...
    def _build_metadata(self, content: EducationalContent) -> list[dict]:
        ...
        if content.author:
            metadata.append({
                'key': 'author',
                'string_value': content.author
            })

        if content.publication_year:
            metadata.append({
                'key': 'year',
                'numeric_value': float(content.publication_year)
            })
    ...
    def _build_system_instruction(self, student_context: dict | None) -> str | None:
        ...
        instruction_parts = ["You are an educational tutor bot. Provide clear, helpful explanations."]

        if student_context.get('grade_level'):
            instruction_parts.append(f"Adapt explanations for {student_context['grade_level']} level.")
        ...
        if student_context.get('learning_style'):
            ...
            if style == 'visual':
                ...
                instruction_parts.append("Explain concepts verbally with clear step-by-step instructions.")
            elif style == 'reading':
                instruction_parts.append("Provide detailed written explanations with examples.")

        if student_context.get('preferred_language') and student_context['preferred_language'] != 'en':
            instruction_parts.append(f"Respond in {student_context['preferred_language']} when appropriate.")
