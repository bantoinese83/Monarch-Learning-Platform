...
class EducationalContentSerializer(serializers.ModelSerializer):
    ...
    file_search_store_name = serializers.CharField(source='file_search_store.display_name', read_only=True)
    ...
    class Meta:
        ...
        fields = [
            'id', 'title', 'description', 'file', 'file_url', 'file_name', 'file_type', 'file_size',
            'subject', 'difficulty', 'author', 'publication_year', 'tags',
            'file_search_store', 'file_search_store_name', 'gemini_file_name',
            'indexed', 'indexing_error', 'version', 'parent_content',
            'uploaded_by', 'uploaded_by_username', 'created_at', 'updated_at',
            'custom_metadata'
        ]
        read_only_fields = [
            'id', 'file_name', 'file_type', 'file_size', 'gemini_file_name',
            'indexed', 'indexing_error', 'uploaded_by', 'created_at', 'updated_at'
        ]
        extra_kwargs = {
            'file': {'required': True},
            'title': {'required': True},
...
class FileSearchStoreSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'name', 'display_name', 'created_by', 'created_at', 'updated_at', 'contents_count']
