...
class EducationalContentViewSet(viewsets.ModelViewSet):
                ...
                if content.file:
                    try:
                        if hasattr(content.file, 'size') and content.file.size:
                            ...
                            try:
                                content.file_size = content.file.size if hasattr(content.file, 'size') else 0
                            except Exception:
                                content.file_size = 0
    ...
    def by_subject(self, request):
        ...
        serializer = EducationalContentSerializer(all_content, many=True, context={'request': request})
    ...
    def extract_metadata(self, request):
        ...
        if file.content_type not in allowed_types and not any(file.name.endswith(ext) for ext in ['.pdf', '.docx', '.txt']):
            return Response(
